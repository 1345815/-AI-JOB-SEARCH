...
class Parser(HTMLParser):
 ...
 def handle_starttag(self,t,a):
  ...
  if t in ("input","select","textarea"): self.controls.append({"id":d.get("id",""),"name":d.get("name","") or d.get("id", ""),"placeholder":d.get("placeholder", "").strip(),"type":d.get("type","select" if t=="select" else "textarea" if t=="textarea" else "text"),"required":"required" in d,"value":d.get("value","")})
...
def extract_form(html, source_url=""):
 ...
 for c in p.controls:
  placeholder=c["placeholder"] if c["placeholder"] and c["placeholder"] != c["name"] else ""
  label=placeholder or p.labels.get(c["id"]) or p.labels.get(c["name"]) or c["name"]; key=next((v for k,v in mapping.items() if k in label),""); item=result.setdefault(c["name"] or c["id"],{"key":key,"label":label,"type":c["type"],"required":c["required"],"options":[],"selector":"#"+c["id"] if c["id"] else "[name='"+c["name"]+"']"})
