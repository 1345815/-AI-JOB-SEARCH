...
> Three ways to start:
>
> **Path A: Documents folder** (best signal if you have several materials) - Drop your CV / LinkedIn export / diplomas / proof documents (internship certificates, awards, etc.) in the `documents/` folder, then say "go". I'll read everything and build your profile from it. See `documents/README.md` for the folder layout.
>
> **Path B: Single CV import** - Paste or @-mention a single CV/resume here. I'll extract it and ask follow-up questions for what's missing.
>
...
**`references/` documents:** issuer/org, document type, date, and the specific achievement or competency language shown in each proof document (internship certificate, award, competition certificate, etc.).
...
**Additive changes:** entirely new content not in the skill file in any form. Examples: a certification not in `01-candidate-profile.md`, a new endorsement skill, a new proof document, a new behavioral signal from a proof document, a new award.
...
- **`02-behavioral-profile.md`:** Source is LinkedIn About + proof documents. Extract recurring themes, adjectives, phrases about how the candidate works. Add only to "Strongest Behavioral Traits", "How [Candidate] Works Best", or "Management Style Preferences" sections. Do not overwrite existing scored assessments. Always label inferred additions: *[Inferred from LinkedIn About / Proof document - review before relying on this]*
...
- **`07-interview-prep.md`:** Source is CV bullets, LinkedIn descriptions, proof documents. Identify achievements not yet covered by an existing STAR example. Do NOT draft full STAR examples. Add stubs under "## STAR Candidates (Complete Manually)":

```markdown
### [Achievement title]
**Source:** [CV / LinkedIn / Proof document - role/company]
...
- [ ] New proof document: [type, issuer, date]
...
Documents cover skills, experience, education, proof documents, and behavioral signal. They do not cover everything `/apply` and `/scrape` need. After the writes, ask follow-up questions for gaps:
...
4. Ask follow-up questions for gaps (behavioral profile, career goals, deal-breakers, languages and proficiency levels if not already extracted, salary expectations, proof documents).
...
### Section 8: Other Proof Documents (optional)
For each supporting document (internship certificate, award, competition certificate):
- Type, issuer, date, and what it demonstrates
...
Write the full candidate profile with structured sections: Identity (including Languages, with levels), Education, Professional Experience, Independent Projects, Technical Skills, Publications, Awards, Other Proof Documents.
