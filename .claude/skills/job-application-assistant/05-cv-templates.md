...
> **Active template override: `china-student`**
>
...
>
> - **Template skeleton:** `templates/cv/china-student/template.tex` - use this as the structural reference instead of the stock template
> - **Manifest:** `templates/cv/china-student/TEMPLATE.md` - read this for style rules and known pitfalls before drafting
> - **Source extension:** `.tex`
> - **Compile command:** `cd cv && xelatex -interaction=nonstopmode <file>.tex`
...
% 7. References（中国校招一般省略）（中国校招一般省略）
...
**For AI游戏策划 roles:**
> 2027届飞行器控制与信息工程本科生，深度玩家与系统解读者（50+款/8+品类游戏体验，SLG重度玩家）。独立开发并上线AI+NLP在线工具「简历匹配分析器」，熟悉ChatGPT/Claude/Coze/Dify/LangChain等AI工具链，具备LLM调用、Prompt设计与容错策略的一手经验。独立完成《我的世界》拔刀剑玩法Demo，验证「战斗爽感→成长目标→长期留存」的玩法链路。长期产出《三国志·战略版》系统拆解案，能把玩法、留存、商业化与社交结构拆成可复用的知识体系。

**For AI产品运营 roles:**
> 2027届本科生，AI产品落地经验丰富——独立开发并上线「简历匹配分析器」（AI+NLP在线工具），从0到1完成产品设计、开发、上线与迭代，真实用户在使用中验证效果。数据驱动运营：用Python/Pandas/Matplotlib做数据分析，独立开发的招聘进度平台通过数据洞察降低用户操作成本约40%。在游戏公司一线推广体系中跑通「推广→注册→转化」获客链路。抖音MCN创作者计划成员+B站UP主，懂用户语言与内容传播规律。

**For 技术/产品复合型 roles:**
> 2027届飞行器控制与信息工程本科生，具备AI产品从0到1落地的完整经验。独立开发并上线多个产品（AI+NLP工具、Web+小程序双端平台、游戏玩法Demo），熟悉LLM调用、Prompt工程、Python数据分析与Web开发。系统工程思维训练+跨学科团队协作经验（5人团队队长，获省级一等奖），擅长将复杂系统拆解为可复用的知识结构。习惯快节奏、高挑战环境，以"做出用户真实使用的产品"为核心动力。
...
### References（中国校招简历一般不放）
- 中国校招简历通常不写 References 段，也不附推荐信；可删除该段，或在文末写一句“实习证明、获奖证书等材料可提供”。
- 如果目标岗位明确要求推荐人/证明人，再按对方要求填写。
...
**Problem: one trailing section spills to page 3 (e.g., Honors and Awards alone on page 3)**
...
| References | 中国校招简历一般省略此段；如需保留，用“证明材料可提供” |
