...
- `cv/`：放你的中文简历主文件（PDF 或 LaTeX），内容越完整，`/setup` 提取的资料越准。
- `diplomas/`：放成绩单、毕业证、学位证扫描件（PDF）。
- `references/`：放其他证明材料，如实习证明、获奖证书、竞赛证书、技能证书等（中国校招一般不需要推荐信）。
- `applications/`：每次投递后的记录会自动归档在这里，投递后可以用 `/outcome` 记录进度。
- 校招季建议把"网申截止日期"写进 `job_search_tracker.csv` 的备注列，方便 `/scrape` 和 `/outcome` 提醒。
...
├── references/                  # Certificates and proof documents
...
- Recommendations received (may enrich behavioral context)
...
Proof documents such as internship certificates, awards, competition certificates, and skills certificates. In Chinese campus recruitment, recommendation letters are not expected; treat this folder as "other supporting materials".

**Supported formats:** `.pdf`, `.txt`, `.md`

**What `/setup` extracts:**
- Issuer/org name, title, and organization (for certificates and awards)
- Specific achievement or assessment text (added to the proof-documents section of `01-candidate-profile.md`)
- Competency language used in the documents (adds behavioral signal to `02-behavioral-profile.md`)

**Naming:** Use descriptive names, e.g. `internship_tencent_2025.pdf`, `award_icpc_2024.pdf`.
...
- After adding new proof documents (internship certificates, awards, etc.)
